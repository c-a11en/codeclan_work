...
diet_comp_clean <- diet_comp %>% 
  pivot_longer(alcoholic_beverages_fao_2017:wheat_fao_2017, names_to = "kcal_source", values_to = "avg_daily_kcals")
...
diet_comp_clean %>% 
  summarise(num_na = sum(is.na(code))) #used ot check if code column has any NAs
...
find_top_source <- function(country, year) {
  ...
  # calculating length of input vectors
  c_len <- length(country)
  y_len <- length(year)
  ...
  for (i in country) { # i is each country
    ...
    for (j in year) { # j is each year
      ...
      # if first element, create first output row, else append to output
      if (i == country[1] & j == year[1]) {
        output <- top_source
      } else {
  ...
  output <- rename(output, "top_kcal_source" = "kcal_source")
  return(output)
}

country <- c("United Kingdom", "Zimbabwe", "Indonesia")
