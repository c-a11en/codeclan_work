...
conf_mat_caret <- confusionMatrix(thrones_test_pred$pred, thrones_test_pred$dth_flag)
...
conf_mat_caret$overall["Accuracy"]
conf_mat_caret
...
```{r}
# creating range of hyperparameters
...
rf_classifier <- ranger(dth_flag ~ .,
                        thrones_train,
                        importance = "impurity",
                        num.trees = 1000, # make 1,000 decision trees
                        mtry = 2, # how many variable decisions at each node
...
Split data 80:20 (test:train)
k-fold validation to select best hyperparameters
Retrain model with training data (train + validation = 80%) using best hyperparams
...
thrones_test_pred_forest <- thrones_test %>% 
  mutate(pred = predict(rf_classifier, thrones_test)$predictions)

thrones_test_pred_forest

```

```{r}

confusionMatrix(thrones_test_pred_forest$pred, thrones_test_pred_forest$dth_flag)
